...
export class Mapbox extends Component {
  ...
  async componentDidMount() {
    ...
    thefeatures.forEach(function(marker) {
      ...
      var sortscountry = marker.properties.products.map(p => p);
      var popup = new mapboxgl.Popup({closeOnClick: false, className: 'my-popup-class'})
        .setLngLat(marker.geometry.coordinates)
        .setHTML('<div><h1  id="id_' + thecountry + '" class="dp_color_' + thecountry + '" style="font-size:' + fontsize + 'px;">' + capcountry + '<span>' + marker.properties.number + '</span></h1></div>')  // <ul class="sortscountry">' + sortscountry + '</ul>
        .addTo(map);
    })
  }
